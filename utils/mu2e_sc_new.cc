...
// Name:         mu2e_sc_new.cc
// P.Murat
// handles multiple drivers, each drive with its own set of history ('input') arrays
// Contents:     slow control Class Driver
//-----------------------------------------------------------------------------
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
struct MU2E_DRIVER_INFO_t {
  ...
  te_expr** expr_input;
  double*   expr_x;
  float*    input_mirror;               // mirror array

  char*     names_output;
  float*    var_output;
  float*    offset_output;
  float*    factor_output;
  float*    output_mirror;              // mirror array
};
...
typedef struct {
  ...
  /* globals */
  INT                  num_drivers ;
  MU2E_DRIVER_INFO_t*  driver_info ;

  INT   num_channels_input;          // total (per equipment) number of input channels
  int   num_channels_output;         // total (per equipment) number of output channels
  INT   format;
  INT   last_channel;
...
#endif
//------------------------------------------------------------------------------
// organize by the driver
// need to keep offsets, as the interface to the class driver assumes 
// a single throughout numbering
// status : OK
//-----------------------------------------------------------------------------
static void free_mem(MU2E_SC_NEW_INFO *m_info) {

  for (int i=0; i<m_info_num_drivers; i++) {
    DEVICE_DRIVER*      drv = &pequipment->driver[i];
    MU2E_DRIVER_INFO_t* di  = m_info->driver[i];
    
    if (drv->flags & DF_INPUT ) {
      free(di->names_input     );
      free(di->var_input       );
      free(di->update_threshold);
      free(di->offset_input    );
      free(di->factor_input    );
      free(di->formula_input   );
      free(di->expr_input      );
      free(di->expr_x          );
      free(di->input_mirror    );
    }

    if (drv->flags & DF_OUTPUT ) {
      free(di->names_output );
      free(di->var_output   );
      free(di->offset_output);
      free(di->factor_output);
      free(di->output_mirror);
    }
  }
...
void mu2e_sc_new_read(EQUIPMENT *pequipment, int channel) {
  ...
  m_info = (MU2E_SC_NEW_INFO *) pequipment->cd_info;
      ...
      if (status != FE_SUCCESS)
        ...
        if (m_info->formula_input[i*FORMULA_SIZE] != 0) {
          if (m_info->expr_input[i] == NULL) {
            int error;
            te_variable vars[] = {{"x", &m_info->expr_x[i],-1,nullptr}};
    ...
    if (status != FE_SUCCESS)
      ...
      if (m_info->formula_input[channel * FORMULA_SIZE] != 0) { // evaluate formula
        if (m_info->expr_input[channel] == NULL) {
          int error;
          te_variable vars[] = {{"x", &m_info->expr_x[channel], -1, nullptr}};
...
#endif
//-----------------------------------------------------------------------------
// update if change is more than update_sensitivity or last update more than a minute ago
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// channel=-1: read everything
// otherwise, 'channel' is the throughout channel number in the equipment 
// output channel list
//-----------------------------------------------------------------------------
...
// read all
//-----------------------------------------------------------------------------
    for (int i = 0; i < m_info->num_channels_output; i++) {

      int loc = i - m_info->channel_offset_output[i];

      if (m_info->driver_output[i]->flags & DF_MULTITHREAD) {
        device_driver(m_info->driver_output[i],CMD_GET_DIRECT,loc,&value);
      }
      else {
        device_driver(m_info->driver_output[i],CMD_GET,loc,&value);
      }
...
//-----------------------------------------------------------------------------
// read single channel - this is where the linear mapping comes in play 
// and the fact that the channel number is throughout is 
// drv->dd is th function which actuallyreads the 
//-----------------------------------------------------------------------------
    DEVICE_DRIVER* drv = m_info->driver_output[channel];
    int loc            = channel-m_info->channel_offset_output[channel];
    device_driver(drv,CMD_GET,loc,&value);
...
//-----------------------------------------------------------------------------
// init a new class driver - multiple single drivers, each with its own arrays
//-----------------------------------------------------------------------------
INT mu2e_sc_new_init(EQUIPMENT *pequipment) {
...
//-----------------------------------------------------------------------------
   pequipment->cd_info = calloc(1, sizeof(MU2E_SC_NEW_INFO));
   m_info = (MU2E_SC_NEW_INFO *) pequipment->cd_info;
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// save event format
// P.M. for slow control-only driver defining the event format may not be needed, but...
//------------------------------------------------------------------------------
   size = sizeof(str);
...
// a driver may have both input and output channels
// need to know the number of input and output variables separately
//-----------------------------------------------------------------------------
   m_info->num_channels_output = 0;
   m_info->num_channels_input  = 0;
   m_info->num_drivers         = 0;
   for (int i = 0; pequipment->driver[i].name[0]; i++) {
     DEVICE_DRIVER* drv   = &pequipment->driver[i];
     m_info->num_drivers += 1;
     if (drv->flags & DF_INPUT ) m_info->num_channels_input  += drv->channels;
     if (drv->flags & DF_OUTPUT) m_info->num_channels_output += drv->channels;
   }
//-----------------------------------------------------------------------------
// sanity check
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// allocate per-driver arrays
// each driver has its own input and output variables
//-----------------------------------------------------------------------------
   m_info->driver_info = nullptr;
   if (m_info->num_drivers > 0) {
     m_info->driver_info =  new MU2E_DRIVER_INFO_t[m_info->num_drivers];
   }
    
   for (int i=0; i<m_info->num_drivers; i++) {
     DEVICE_DRIVER* drv     = &pequipment->driver[i];
     MU2E_DRIVER_INFO_t* di = &m_info->driver_info[i];
     int nch = drv->channels;
     if (nch > 0) {
//-----------------------------------------------------------------------------
// for now , I asume that the channels are either input or output, 
/// but in principle, they could be both
// a 'simple' driver have only one number of channels so assume that 
// a channel can be an 'input' and an 'output' simultaneously
//-----------------------------------------------------------------------------
       if (drv->flags & DF_INPUT ) {
         di->names_input           = (char    *) calloc(nch, NAME_LENGTH   );
         di->var_input             = (float   *) calloc(nch, sizeof(float) );
         di->update_threshold      = (float   *) calloc(nch, sizeof(float) );
         di->offset_input          = (float   *) calloc(nch, sizeof(float) );
         di->factor_input          = (float   *) calloc(nch, sizeof(float) );
         di->formula_input         = (char    *) calloc(nch, FORMULA_SIZE  );
         di->expr_input            = (te_expr**) calloc(nch, sizeof(void*) );
         di->expr_x                = (double*  ) calloc(nch, sizeof(double));
         di->input_mirror          = (float*   ) calloc(nch, sizeof(float) );
         di->channel_offset_input  = (INT*     ) calloc(nch, sizeof(INT)   );
         di->driver_input          = (DEVICE_DRIVER **) calloc(nch, sizeof(void*));
       }

       if (drv->flags & DF_OUTPUT) {
         di->names_output          = (char*    ) calloc(nch, NAME_LENGTH  );
         di->var_output            = (float *  ) calloc(nch, sizeof(float));
         di->offset_output         = (float *  ) calloc(nch, sizeof(float));
         di->factor_output         = (float *  ) calloc(nch, sizeof(float));
         di->output_mirror         = (float *  ) calloc(nch, sizeof(float));
         di->channel_offset_output = (INT*     ) calloc(nch, sizeof(DWORD));
         di->driver_output         = (DEVICE_DRIVER **) calloc(nch, sizeof(void*));
       }
     }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
     ...
     // int nch = m_info->num_channels_output;
     // if (nch) {
     //   m_info->names_output          = (char  *) calloc(nch, NAME_LENGTH);
     //   m_info->var_output            = (float *) calloc(nch, sizeof(float));
     //   m_info->offset_output         = (float *) calloc(nch, sizeof(float));
     //   m_info->factor_output         = (float *) calloc(nch, sizeof(float));
     //   // P.Murat: is it an offset per channel? 
     //   m_info->output_mirror         = (float          *) calloc(nch, sizeof(float));
     //   m_info->channel_offset_output = (INT            *) calloc(nch, sizeof(DWORD));
     //   m_info->driver_output         = (DEVICE_DRIVER **) calloc(nch, sizeof(void *));
     // }
//-----------------------------------------------------------------------------
...
//   if (m_info->num_channels_input) {
//-----------------------------------------------------------------------------
// 1. Update thresholds: default 0.1
//-----------------------------------------------------------------------------
     for (int i=0; i<m_info->num_drivers; i++) {
       DEVICE_DRIVER*      drv = &pequipment->driver[i];
       MU2E_DRIVER_INFO_t* di  = &m_info->driver_info[i];
       if (drv->flags & DF_INPUT ) {
         for (int ich = 0; ich < drv->channels; ich++) {
           di->update_threshold[ich] = 0.1f;
         }
...
//-----------------------------------------------------------------------------
       ...
       if (m_info->num_channels_input) {
         ...
         db_open_record(hDB, hKey, m_info->formula_input,
                        m_info->num_channels_input * FORMULA_SIZE, MODE_READ, formula_invalidate_new, m_info);
       }
...
//-----------------------------------------------------------------------------
       ...
       for (i = 0; pequipment->driver[i].name[0]; i++) {
         ...
         if (status != DB_SUCCESS) {
           ...
           status = db_find_key(hDB, m_info->hKeyRoot, str, &hKey);
           if (status != DB_SUCCESS) {
             cm_msg(MERROR, "mu2e_sc_new_init", "Cannot create %s entry in online database",str);
             free_mem(m_info);
             return                                             FE_ERR_ODB;
           }
         }
...
// separate input and output lists, need to do that intelligently
// for each variable, there is a backward pointer to the driver
// 'j' counts the number of channels of a given driver
// driver_input and driver_output have their lengths equal to the total (per 'equipment')
// numbers of 'input' and 'output' channels
//-----------------------------------------------------------------------------
       for (i = 0, j = 0, index = 0, ch_offset = 0; i < m_info->num_channels_input; i++, j++) {
         DEVICE_DRIVER* drv = &pequipment->driver[index];
         while (drv->name[0] && (j >= drv->channels || (drv->flags & DF_INPUT) == 0)) {
           ch_offset += j;
           index++;
           j = 0;
         }

         m_info->driver_input        [i] = drv;
         m_info->channel_offset_input[i] = ch_offset;
       }

       for (i = 0, j = 0, index = 0, ch_offset = 0; i < m_info->num_channels_output; i++, j++) {
         DEVICE_DRIVER* drv = &pequipment->driver[index];
         while (drv->name[0] && (j >= drv->channels || (drv->flags & DF_OUTPUT) == 0)) {
           ch_offset += j;
           index++;
           j = 0;
         }
...
//-----------------------------------------------------------------------------
// get default input variable names (labels) from the device driver 
//-----------------------------------------------------------------------------
       if (m_info->num_channels_input) {
         for (i = 0; i < m_info->num_channels_input; i++) {
...
//-----------------------------------------------------------------------------
// get default output variable names (labels) from the device driver 
//-----------------------------------------------------------------------------
       if (m_info->num_channels_output) {
         for (i = 0; i < m_info->num_channels_output; i++) {
...
//-----------------------------------------------------------------------------
         if (db_find_key(hDB, m_info->hKeyRoot, "Settings/Names Input", &hNamesIn) ==
           ...
           db_open_record(hDB, hNamesIn, m_info->names_input,
                          ...
                          mu2e_sc_new_update_label, pequipment);
       }
   ...
   if (m_info->num_channels_output) {
      ...
      db_open_record(hDB, m_info->hKeyOutput, m_info->var_output,
                     ...
                     MODE_READ, mu2e_sc_new_output, pequipment);
   }
//-----------------------------------------------------------------------------
...
INT mu2e_sc_new_exit(EQUIPMENT *pequipment) {
   INT i;

   free_mem((MU2E_SC_NEW_INFO *) pequipment->cd_info);
...
/*----------------------------------------------------------------------------*/
 ...
 INT mu2e_sc_new_start(EQUIPMENT *pequipment) {
   INT i;
...
//-----------------------------------------------------------------------------
INT mu2e_sc_new_stop(EQUIPMENT *pequipment) {
  for (int i = 0; ((pequipment->driver[i].dd != NULL) && (pequipment->driver[i].flags & DF_MULTITHREAD)); i++) {
...
// 
//-----------------------------------------------------------------------------
INT mu2e_sc_new_idle(EQUIPMENT *pequipment) {
  MU2E_SC_NEW_INFO *m_info;
...
//-----------------------------------------------------------------------------
  mu2e_sc_new_read(pequipment,-1);
//-----------------------------------------------------------------------------
// and output channels for 'output' drivers
//-----------------------------------------------------------------------------
  for (int i = 0; i < m_info->num_dirvers; i++) {
    if (m_info->driver_output[i]->flags & DF_PRIO_DEVICE) {
      mu2e_sc_new_read_output(pequipment,i);
    }
  }
...
// status: wrong, but is not going to be used
//-----------------------------------------------------------------------------
...
// status: OK
//-----------------------------------------------------------------------------
INT cd_mu2e_sc_new(INT cmd, EQUIPMENT *pequipment) {
  INT status;

  switch (cmd) {
  case CMD_INIT:
    status = mu2e_sc_new_init(pequipment);
    break;

  case CMD_EXIT:
    status = mu2e_sc_new_exit(pequipment);
    break;

  case CMD_START:
    status = mu2e_sc_new_start(pequipment);
    break;

  case CMD_STOP:
    status = mu2e_sc_new_stop(pequipment);
    break;

  case CMD_IDLE:
    status = mu2e_sc_new_idle(pequipment);
    break;

  default:
    cm_msg(MERROR, "mu2e_sc_new class driver", "Received unknown command %d", cmd);
