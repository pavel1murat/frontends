...
#+end_src
# ------------------------------------------------------------------------------
# 0.1 comment out everything *-suite related in mu2e and artdaq recipes - TOD : need to autonmate
# 0.2 comment out dependence on otsdaq-mu2e in otsdaq-mu2e-tracker
#
# 0.3 to handle (a spack/Eric of a spack 0.23 error?) declare gcc @11.5.0 as a package
#     in ~/.spack/bootstrap/config/packages.yaml, otherwise, it doesn't concretize
#
# 0.4 add CPU target (today - x86_64_v3) to the $spack_env/spack.yaml, also use system-wide krb5, openssl, and opengl
# 0.5 keep using gcc 13.1.0 , problems otherwise.
...
#    after copying spack.yaml, edit it to make sure the directory names are correct
#    that boils down to changind daquser_002 -> daquser_003 (can be automated)
# ------------------------------------------------------------------------------
...
# spack add     node-js ## already in copied spack.yaml
# ------------------------------------------------------------------------------
