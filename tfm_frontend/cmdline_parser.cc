...
static optionx* createLongOptsArray(optionDesc*  const optionDescArray,
                                    unsigned int const numOptions     ) {

  struct optionx* longopts;

    //MALLOCARRAY(longopts, numOptions+1);
    mallocProduct((void**) &longopts, numOptions+1, sizeof(longopts[0]));
    ...
    if (longopts != NULL) {
        ...
        longopts[numOptions].has_arg = argreq::no_argument;
...
extractArguments(struct cmdlineParserCtl * const cpP,
    ...
    mallocProduct((void**) &cpP->argumentArray, cpP->numArguments, sizeof(cpP->argumentArray[0]));
...
cmdlineParser cmd_createOptionParser(void) {

  cmdlineParserCtl * cpP = (cmdlineParserCtl*) malloc(sizeof(cmdlineParserCtl));
  ...
  if (cpP != NULL) {
    ...
    mallocProduct((void**) &optionDescArray, MAXOPTS, sizeof(optionDescArray[0]));
...
void cmd_destroyOptionParser(cmdlineParser const cpP) {

  for (uint i = 0; i < cpP->numOptions; ++i) {
  ...
  for (uint i = 0; i < cpP->numArguments; ++i) {
...
void cmd_defineOption(cmdlineParser   const cpP,
...
unsigned int cmd_argumentCount(cmdlineParser const cpP) {
