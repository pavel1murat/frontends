///////////////////////////////////////////////////////////////////////////////
// this is a tracker command - it doesn't require accessing the DTC's or RPI's
///////////////////////////////////////////////////////////////////////////////
#include <format>
...
//-----------------------------------------------------------------------------
// as the tracker normall fans out the commands, the status is set to BUSY by  ProcessCommand
// and, if everything goes well, restored by WaitForCompletion
int TEqTracker::PanelPrintStatus(HNDLE H_Cmd) {
  int rc(0);
...
//-----------------------------------------------------------------------------
  ...
  int   mnid     = _odb_i->GetInteger(H_Cmd,"mnid"); // o_params["mnid"];
  HNDLE h_panel  = _odb_i->GetTrackerPanelHandle(mnid);
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
  ...
  _odb_i->GetArray(h_panel,"thr_hv_mv" ,TID_FLOAT, thr_hv_mv ,96);
  _odb_i->GetArray(h_panel,"thr_cal_mv",TID_FLOAT, thr_cal_mv,96);
