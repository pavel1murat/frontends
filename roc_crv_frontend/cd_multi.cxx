...
void formula_invalidate(INT hDB, INT hKey, void *info) {
   MULTI_INFO *m_info = (MULTI_INFO *) info;
...
INT cd_multi_set_register(EQUIPMENT * pequipment, int32_t add, int32_t val) {

  MULTI_INFO *m_info;
  m_info = (MULTI_INFO *) pequipment->cd_info;

  device_driver(m_info->driver_output[0], 
                CMD_SET+1000,
                add,
                val);
...
INT cd_multi_get_register(EQUIPMENT * pequipment, int32_t add, int32_t * val) {


  MULTI_INFO *m_info;
  m_info = (MULTI_INFO *) pequipment->cd_info;

  device_driver(m_info->driver_input[0], 
                CMD_GET+1000,
                add,
                val);
